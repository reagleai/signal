    ...
    return (
        <header className="h-14 sm:h-16 bg-[#232F3E] px-4 sm:px-8 flex items-center justify-between shadow-[0_2px_8px_rgba(0,0,0,0.25)] flex-shrink-0 z-50">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
                ...
                {/* Sub-label */}
                <div className="flex flex-col justify-center ml-2 md:ml-1 border-l border-white/20 pl-2 md:pl-3">
                    <span className="hidden md:block text-[11px] text-white/60">Returns Discovery</span>
                    {onBackToLanding ? (
                        <button onClick={onBackToLanding} className="text-[12px] md:text-[10px] text-[#FF9900]/90 hover:text-[#FF9900] transition-colors text-left cursor-pointer font-medium md:font-normal">
                            ← Back to Home
                        </button>
                    ) : (
                        <span className="hidden md:block text-[10px] text-white/50">Internal Tool</span>
                    )}
                </div>
            </div>
